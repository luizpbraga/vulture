...
fn init(ally: std.mem.Allocator) Vulture {
    return .{
        .ally = ally,
...
fn deinit(self: *Vulture) void {
    self.server.deinit();
    self.routes.deinit();
}

fn newRoute(self: *Vulture, method: http.Method, target: []const u8, handleFunc: RouteFunc) !void {
    try self.routes.put(target, .{ .method = method, .handleFunc = handleFunc });
}

fn get(self: *Vulture, target: []const u8, handleFunc: RouteFunc) !void {
    try self.routes.put(target, .{ .method = .GET, .handleFunc = handleFunc });
}

fn post(self: *Vulture, target: []const u8, handleFunc: RouteFunc) !void {
    try self.routes.put(target, .{ .method = .POST, .handleFunc = handleFunc });
}

fn listen(self: *Vulture, address: []const u8, port: u16) !void {
